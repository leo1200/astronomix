...
fig.suptitle(f'Sedov Blast Wave at t = {params.t_end:.2f} (Grid: {config.num_cells.x}³)', fontsize=16)
...
plt.savefig('figures/sedov_profiles.png', dpi=300)
print("Plot with binned profiles and JAX-sampled scatter saved as 'sedov_profiles.png'.")

# also plot the final density, abs. velocity and pressure
# at z_index = num_cells // 2 (the midplane of the explosion)
fig, axes = plt.subplots(1, 3, figsize=(12, 4))
z_index = config.num_cells.z // 2
extent = [0, config.box_size.x, 0, config.box_size.y]
im = axes[0].imshow(result[registered_variables.density_index][:, :, z_index].T, origin='lower', extent=extent, cmap='viridis')
axes[0].set_title('Density Slice (z = midplane)')
fig.colorbar(im, ax=axes[0], label='Density')
im = axes[1].imshow(result[registered_variables.velocity_index.x][:, :, z_index].T, origin='lower', extent=extent, cmap='plasma')
axes[1].set_title('Velocity x Slice (z = midplane)')
fig.colorbar(im, ax=axes[1], label='Velocity x')
im = axes[2].imshow(result[registered_variables.pressure_index][:, :, z_index].T, origin='lower', extent=extent, cmap='inferno')
axes[2].set_title('Pressure Slice (z = midplane)')
fig.colorbar(im, ax=axes[2], label='Pressure')
plt.tight_layout()
plt.savefig('figures/sedov_slices.png', dpi=300)
print("Midplane slices saved as 'sedov_slices.png'.")
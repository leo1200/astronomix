...
from astronomix._finite_difference._maths._interpolate import (
...
# CHANGE: KEEP VY AND VZ IN MHD SIMULATIONS, EVEN IN 1D and 2D!!!
...
def constrained_transport_rhs(
    ...
    if config.dimensionality == 1:
        ...
        By_vx = By * vx
        By_vz = By * vz
        Bx_flux_y_interface_mod = Bx_flux_y_interface + interp_center_to_face(
            By_vx, YAXIS
        )
        Bz_flux_y_interface_mod = Bz_flux_y_interface + interp_center_to_face(
            By_vz, YAXIS
        )
    ...
    if config.dimensionality <= 2:
        ...
        Bz_vx = Bz * vx
        Bz_vy = Bz * vy
        Bx_flux_z_interface_mod = Bx_flux_z_interface + interp_center_to_face(
            Bz_vx, ZAXIS
        )
        By_flux_z_interface_mod = By_flux_z_interface + interp_center_to_face(
            Bz_vy, ZAXIS
        )
...
def update_cell_center_fields(
    ...
    if config.equation_of_state == IDEAL_GAS:
        b2_old = (
            conserved_state[BX] ** 2
            + conserved_state[BY] ** 2
            + conserved_state[BZ] ** 2
        )
    ...
    if config.equation_of_state == IDEAL_GAS:
        b2_new = (
            conserved_new[BX] ** 2 + conserved_new[BY] ** 2 + conserved_new[BZ] ** 2
        )

"""Validate the refactored ideal-gas MHD WENO Pallas forward kernel (now built
on the shared pure ``_weno_mhd_flux_from_window``) against the native per-axis
JAX flux.  Must stay bit-exact (the refactor only factored the inlined window
arithmetic into a function; no math changed).  3D, all three axes.
...
def main():
    N = 16
    cfg, params, rv, q = _make(N)
    assert _mhd_pallas_flux_supported(q, cfg), "MHD Pallas predicate False!"
    ok = True
    for axis in range(3):
        native = _NATIVE[axis](q, params, cfg, rv)
        pallas = _weno_flux_mhd_pallas_local(q, params, cfg, rv, axis=axis)
        print(f"  axis={axis}")
        ok &= _cmp("forward", pallas, native)

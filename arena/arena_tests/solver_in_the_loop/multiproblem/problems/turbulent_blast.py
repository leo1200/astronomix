...
class TurbulentBlast(BaseProblem):
    def __init__(
        self,
        B0: float = 0.7071,
        r0: float = 0.125,
        ...
        self.wanted_rms = wanted_rms
        self.P0 = P0

    @property
    def name(self) -> str:
        return "turbulent_blast"
    ...
    def generate_initial_state(
        ...
        wanted_rms = self.wanted_rms * u.km / u.s
        ...
        # base pressure with central blast
        r = helper_data.r
        r0 = self.r0
        r1 = 1.1 * self.r0

        p_base = p_0.to(code_units.code_pressure).value
        P = jnp.ones_like(r) * p_base
        P = jnp.where(r <= r0, self.P0 * p_base, P)
        P = jnp.where(
            (r > r0) & (r <= r1),
            p_base + self.P0 * p_base * (r1 - r) / (r1 - r0),
            P,
        )
        ...
        u_x = jnp.where(r < r0, 0, u_x)
        u_x = jnp.where((r > r0) & (r <= r1), u_x * (r1 - r) / (r1 - r0), u_x)
        u_y = jnp.where(r < r0, 0, u_y)
        u_y = jnp.where((r > r0) & (r <= r1), u_y * (r1 - r) / (r1 - r0), u_y)
        u_z = jnp.where(r < r0, 0, u_z)
        u_z = jnp.where((r > r0) & (r <= r1), u_z * (r1 - r) / (r1 - r0), u_z)
...
if __name__ == "__main__":
    ...
    blast = TurbulentBlast(rng_seed=111, P0=200)
    ...
    plot_states(
        ...
        folder="results/turbulent_blast",
    ...
    plot_and_animate_states_with_diagonal(
        ...
        slice_axis="x",
        save_path="results/turbulent_blast",
        file_name="blast_video_p0_200",
        fps=10,
    )

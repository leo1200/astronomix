...
def _eigen_x(
    ...
    lambdas = jnp.stack(
        [vx - lf_c, vx - la_c, vx - ls_c, vx, vx + ls_c, vx + la_c, vx + lf_c], axis=0
    )
    ...
    L = lset(
        0,
        idx_D,
        af * (gam1 * vv2_i + lf_i * vx_i)
        - as_ * ls_i * (bty * vy_i + btz * vz_i) * sgnBx,
    )
    ...
    L = lset(
        2,
        idx_D,
        as_ * (gam1 * vv2_i + ls_i * vx_i)
        + af * lf_i * (bty * vy_i + btz * vz_i) * sgnBx,
    )
    ...
    R = rset(
        idx_E,
        0,
        af * (lf_i**2 - lf_i * vx_i + 0.5 * vv2_i - gam2 * cs2_i)
        + as_ * ls_i * (bty * vy_i + btz * vz_i) * sgnBx,
    )
    ...
    R = rset(
        idx_E,
        2,
        as_ * (ls_i**2 - ls_i * vx_i + 0.5 * vv2_i - gam2 * cs2_i)
        - af * lf_i * (bty * vy_i + btz * vz_i) * sgnBx,
    )
    ...
    sgnBt = jnp.where(
        By_i != 0.0,
        jnp.where(By_i >= 0.0, 1.0, -1.0),
        jnp.where(Bz_i >= 0.0, 1.0, -1.0),
    )
    mask_cs_ge_la = cs_i >= la_i

    # apply sign flips to L
    L = L.at[2, :, ...].set(
        jnp.where(mask_cs_ge_la, L[2, :, ...] * sgnBt, L[2, :, ...])
    )
    L = L.at[4, :, ...].set(
        jnp.where(mask_cs_ge_la, L[4, :, ...] * sgnBt, L[4, :, ...])
    )
    L = L.at[0, :, ...].set(
        jnp.where(~mask_cs_ge_la, L[0, :, ...] * sgnBt, L[0, :, ...])
    )
    L = L.at[6, :, ...].set(
        jnp.where(~mask_cs_ge_la, L[6, :, ...] * sgnBt, L[6, :, ...])
    )

    # apply sign flips to R
    R = R.at[:, 2, ...].set(
        jnp.where(mask_cs_ge_la, R[:, 2, ...] * sgnBt, R[:, 2, ...])
    )
    R = R.at[:, 4, ...].set(
        jnp.where(mask_cs_ge_la, R[:, 4, ...] * sgnBt, R[:, 4, ...])
    )
    R = R.at[:, 0, ...].set(
        jnp.where(~mask_cs_ge_la, R[:, 0, ...] * sgnBt, R[:, 0, ...])
    )
    R = R.at[:, 6, ...].set(
        jnp.where(~mask_cs_ge_la, R[:, 6, ...] * sgnBt, R[:, 6, ...])
    )
